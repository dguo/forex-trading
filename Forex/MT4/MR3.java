...
//----
  ...
  int numberOfOrdersLeft = OrdersTotal();
  for (int a = 0; a < numberOfOrdersLeft; a++) {
    OrderSelect(0, SELECT_BY_POS);
    OrderClose(OrderTicket(), OrderLots(), Bid, 200, Red);
  }
  SendMail(DoubleToStr(AccountBalance(), 2), StringConcatenate("Orders: ", numberOfOrdersLeft + 1, ", PositionSize: ", PositionSize));  // send an email
//----
}
...
//----
  ...
  for (int x = 0; x < total; x++) {
    ...
    double ProductNew = SizeNew * OrderOpenPrice();
