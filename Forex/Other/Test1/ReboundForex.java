...
public class ReboundForex {
  public static void main(String[] args) {
    ...
    for (int i = start[m]; i < end[m]; i++) {
      
      for (int b = 0; b < 16; b++) {
        if (index == b) {
          if (up) {
            if (data[b][i] < 0) {
              pipTotal = pipTotal + (-1*data[b][i]);
              totalTrades++;
              index = -1;
            }
            else {
              pipTotal = pipTotal - data[b][i];
            }
          }
          else {
            if (data[b][i] > 0) {
      ...
      if (index == -1) {
        ...
        for (int j = 0; j < 16; j++) {
          ...
          if (data[j][i] < 0) {
            int back = i; 
            winnerUp = false;
            while (back > 0 && data[j][back] < 0) {
              tempCounter++;
              tempPipCounter = tempPipCounter + data[j][back];
              back--;
            }
          }
          ...
          if (tempCounter > winnerCounter) {
            winner = j;
            winnerCounter = tempCounter;
            pipCounter = tempPipCounter;
            winnerUp = tempWinnerUp;
          }
          if (tempCounter == winnerCounter && tempPipCounter > pipCounter) {
            winner = j;
            pipCounter = tempPipCounter;
            winnerUp = tempWinnerUp;
          }
